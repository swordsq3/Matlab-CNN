...
imageDim = double(28);         % image dimension
filterDim = double(8);          % filter dimension
numFilters = double(100);         % number of feature maps
numImages = double(60000);    % number of images
poolDim = double(3);          % dimension of pooling region
...
pooledFeatures = cnnPool([poolDim poolDim], convolvedFeatures);
...
pooledFeatures = squeeze(cnnPool([4 4], testMatrix));
